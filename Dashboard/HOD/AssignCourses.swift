        ...
        VStack {
            ...
            VStack{
                                ...
                                VStack {
                                    Button(action: {
                                        ...
                                        if assignedcoursesViewModel.isCourseAssigned(courseID: cr.c_id) {
                                            ...
                                            selectedCourses.remove(cr.c_id)
                                        }
                                    }) {
                                        Image(systemName: selectedCourses.contains(cr.c_id) || assignedcoursesViewModel.isCourseAssigned(courseID: cr.c_id) ? "checkmark.square.fill" : "square")
                                            .font(.title2)
                ...
                .onAppear {
                    ...
                    if let selectedFacultyID = facultiesViewModel.remaining.first?.f_id {
                        assignedcoursesViewModel.fetchAssignedCourses(facultyID: selectedFacultyID)
                        
                        // Check if the selected faculty has no assigned courses
